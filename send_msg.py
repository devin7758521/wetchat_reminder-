...
# ===================== 配置信息 (已硬编码) =====================
# 直接使用你的 Webhook Key，无需在 GitHub Secrets 设置
...
def check_strategy(code, retries=2):
    """带重试机制的 5/60日均量线粘合向上策略"""
    for _ in range(retries):
        try:
            # 防封锁随机延迟
            time.sleep(random.uniform(0.1, 0.3))
            
            # 获取 120 天行情（计算 60 日均线需要足够长度）
...
def main():
    ...
    if stocks.empty:
        send_wechat(f"❌ 警告: 所有接口均无法获取个股列表，请检查网络！")
        return
